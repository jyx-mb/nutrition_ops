...
# ===== NEW ① (import) — goes right after the imports =====
#/ bring in the recommender + the food table solver already loaded
from solver import recommend, foods    ## recommend() = the LP; foods = the table, reused so we don't re-read the CSV
...
# ===== NEW (request body) — goes right after PredictRequest =====
...
# ===== NEW (the route) — goes at the very bottom of the file =====
#/ POST food ids -> grams to hit every floor, or an honest "can't"
@app.post("/recommend")
def recommend_route(req: RecommendRequest):                    
    #/ guard 1: empty list = nothing to optimiz
    if len(req.nummers) == 0:
        raise HTTPException(status_code=422, detail="give at least one food id")

    #/ guard 2: reject ids i don't have (like /predict rejects missing features)
